...
Package.prototype.url = function(file){
  ...
  var remoteTypes = {
    github: function (url, namespace, name, version, file) {
      return url + '/' + namespace + '/' + name + '/' + version + '/' + file;
    },
    stash: function (url, namespace, name, version, file) {
      return url + '/projects/' + namespace + '/repos/' + name + '/browse/' + file + '?at=' + version + '&raw';
    }
  };

  if (remoteTypes[this.remote.type]) {
    return remoteTypes[this.remote.type](this.remote.url, name[0], name[1], this.version, file);
  }
  throw new Error('Remote Type not known:' + this.remote.type);
};
...
Package.prototype.getJSON = function(fn){
  ...
  if (this.remote.auth) {
    req.auth(this.remote.auth.user, this.remote.auth.password);
  }
  ...
  files.forEach(function(file){
    batch.push(function(done){
      ...
      self.mkdir(dirname(dst), function(err){
        ...
        if (self.remote.auth) {
          req.auth(self.remote.auth.user, self.remote.auth.password);
        }
